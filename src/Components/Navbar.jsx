  ...
  return (
    <div className='absolute top-0 left-0 w-full z-10 '>
        <div className='container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32 bg-transparent'>
            ...
            <img src={assets.menu_icon} alt="" className='md:hidden w-7' onClick={ () => { setShowMobileMenu(true) } } />
        </div> 
        {/* ---------- mobile-menu ---------- */}
        <div className={`md:hidden ${showMobileMenu ?  'fixed w-full' : 'h-0 w-0'} right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
          ...
          <ul className='flex flex-col items-center gap-2 mt-5 mx-5 text-lg font-medium'>
            ...
            <a href="" className='px-4 py-2 rounded-full inline-block' onClick={ () => { setShowMobileMenu(false) } } >Project</a>
            <a href="" className='px-4 py-2 rounded-full inline-block' onClick={ () => { setShowMobileMenu(false) } } >Testimonials</a>
          </ul>
        </div>
    </div>
  )
